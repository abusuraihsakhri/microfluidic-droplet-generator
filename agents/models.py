"""
Pydantic v2 schemas and data definitions for Microfluidic Droplet Generator.
...
class SystemTaskPayload(BaseModel):
    ...
    @classmethod
    def reject_nan_and_infinity(cls, v: float) -> float:
        if math.isnan(v) or math.isinf(v):
            raise ValueError("Metric values must be finite numbers (NaN/Infinity rejected)")
        return v
...
class ConsensusDossier(BaseModel):
    dossier_id: str
    system_slug: str = "microfluidic-droplet-generator"
