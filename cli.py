"""
Command Line Interface for Microfluidic Droplet Generator.
"""
import argparse
import csv
import sys
...
def _safe_resolve_path(file_path: str, must_exist: bool = False) -> Path:
    ...
    # Prevent path traversal: ensure the resolved path is under current working directory
    try:
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="microfluidic-droplet-generator", description="Microfluidic Droplet Generator")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  MICROFLUIDIC DROPLET GENERATOR")
    ...
    if args.command == "chat":
        ...
        print(f"\n[Microfluidic Droplet Generator Supervisor]:\n{ans}\n")
        return 0
    ...
    if args.command == "serve":
        ...
        print(f"Starting Microfluidic Droplet Generator API server on http://{args.host}:{args.port}")
